...
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql://root:@localhost/@jit blog'

db = SQLAlchemy(app)

#sl no, name, phone num, email, message, date
...
def home():
    ...
    if (page==1):
        ...
        prev ="/?page=" + str(page-1)
        next = "/?page=" + str(page+1)
    return render_template('index.html', params=params, posts=posts, date= datetime.now(), prev=prev, next=next)
...
def contact():
    if(request.method == 'POST'):
        ...
        db.session.commit()
        mail.send_message('A New message Droped in # @jit blog by @  ' + name,
                          sender=email,
